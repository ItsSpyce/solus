...
namespace impl
{
template <typename Value, bool byRef> class ValueType
{
public:
  using ValueT = Value;
};

template <typename Value> class ValueType<Value, true>
{
public:
  using ValueT = Value &&;
};
...
template <typename Value, bool byRef> class MatchHelper
{
private:
...
public:
  template <typename V> constexpr explicit MatchHelper(V &&value) : mValue{ std::forward<V>(value) }
  {
  }
  template <typename... PatternPair> constexpr auto operator()(PatternPair const &...patterns)
  {
    return matchPatterns(std::forward<ValueRefT>(mValue), patterns...);
  }
};

template <typename Value> constexpr auto match(Value &&value)
{
  return MatchHelper<Value, true>{ std::forward<Value>(value) };
}

template <typename First, typename... Values> constexpr auto match(First &&first, Values &&...values)
{
...
namespace impl
{
template <typename T> class Nullary : public T
{
public:
  using T::operator();
};

template <typename T> constexpr auto nullary(T const &t)
{
  return Nullary<T>{ t };
}

template <typename T> class Id;
template <typename T> constexpr auto expr(Id<T> &id)
{
  return nullary([&] { return *id; });
}

template <typename T> constexpr auto expr(T const &v)
{
  return nullary([&] { return v; });
}

template <typename T> constexpr auto toNullary(T &&v)
{
...
// for constant
template <typename T> class EvalTraits
{
public:
  template <typename... Args> constexpr static decltype(auto) evalImpl(T const &v, Args const &...)
  {
    return v;
  }
};

template <typename T> class EvalTraits<Nullary<T>>
{
public:
...
template <typename T> class EvalTraits<Id<T>>
{
public:
...
template <typename T> using Unary = Meet<T>;

template <typename T> class EvalTraits<Unary<T>>
{
public:
  template <typename Arg> constexpr static decltype(auto) evalImpl(Unary<T> const &e, Arg const &arg)
  {
    return e(arg);
  }
};
...
class Wildcard;
template <> class EvalTraits<Wildcard>
{
public:
  template <typename Arg> constexpr static decltype(auto) evalImpl(Wildcard const &, Arg const &arg)
  {
    return arg;
  }
};

template <typename T, typename... Args> constexpr decltype(auto) evaluate_(T const &t, Args const &...args)
{
  return EvalTraits<T>::evalImpl(t, args...);
}

template <typename T> class IsNullaryOrId : public std::false_type
{
};

template <typename T> class IsNullaryOrId<Id<T>> : public std::true_type
{
};

template <typename T> class IsNullaryOrId<Nullary<T>> : public std::true_type
{
};

template <typename T> constexpr auto isNullaryOrIdV = IsNullaryOrId<std::decay_t<T>>::value;

#define UN_OP_FOR_NULLARY(op)                                                                                    \
  template <typename T, std::enable_if_t<isNullaryOrIdV<T>, bool> = true> constexpr auto operator op(T const &t) \
  {                                                                                                              \
...
// Unary
template <typename T> class IsUnaryOrWildcard : public std::false_type
{
};

template <> class IsUnaryOrWildcard<Wildcard> : public std::true_type
{
};

template <typename T> class IsUnaryOrWildcard<Unary<T>> : public std::true_type
{
};

template <typename T> constexpr auto isUnaryOrWildcardV = IsUnaryOrWildcard<std::decay_t<T>>::value;

// unary is an alias of meet.
template <typename T> constexpr auto unary(T &&t)
{
  return meet(std::forward<T>(t));
}

#define UN_OP_FOR_UNARY(op)                                                                                          \
  template <typename T, std::enable_if_t<isUnaryOrWildcardV<T>, bool> = true> constexpr auto operator op(T const &t) \
  {                                                                                                                  \
...
public:
  ...
  constexpr Subrange(Subrange const &other) : mBegin{ other.begin() }, mEnd{ other.end() }
  {
  }
...
template <typename ElemT, size_t size> class IterUnderlyingType<std::array<ElemT, size>>
{
public:
...
// force array iterators fallback to pointers.
template <typename ElemT, size_t size> class IterUnderlyingType<std::array<ElemT, size> const>
{
public:
...
template <typename I, typename S> bool operator==(Subrange<I, S> const &lhs, Subrange<I, S> const &rhs)
{
  using std::operator==;
...
template <typename T, typename... Ts> class PrependUnique<T, std::tuple<Ts...>>
{
...
template <typename T, typename Tuple> using PrependUniqueT = typename PrependUnique<T, Tuple>::type;

template <typename Tuple> class Unique;

template <typename... Ts> using UniqueT = typename Unique<std::tuple<Ts...>>::type;
...
// [start, end)
template <std::size_t start, std::size_t end, class Tuple> constexpr decltype(auto) subtuple(Tuple &&t)
{
...
template <std::size_t start, class Tuple> constexpr decltype(auto) drop(Tuple &&t)
{
  constexpr auto tupleSize = std::tuple_size_v<std::remove_reference_t<Tuple>>;
...
template <std::size_t len, class Tuple> constexpr decltype(auto) take(Tuple &&t)
{
  constexpr auto tupleSize = std::tuple_size_v<std::remove_reference_t<Tuple>>;
...
template <class F, class Tuple> constexpr decltype(auto) apply_(F &&f, Tuple &&t)
{
...
constexpr std::invoke_result_t<F, Args...> invoke_(F &&f,
...
public:
  using type = typename std::conditional_t<std::is_array<U>::value, typename std::remove_extent<U>::type *, T>;
};
...
template <typename T> using AddConstToPointerT = typename AddConstToPointer<T>::type;
...
template <typename Pattern> using InternalPatternT = std::remove_reference_t<AddConstToPointerT<decayArrayT<Pattern>>>;
...
template <typename Pattern> constexpr void processId(Pattern const &pattern, int32_t depth, IdProcess idProcess)
{
...
template <typename... Ts> using UniqVariant = typename Variant<UniqueT<Ts...>>::type;
...
public:
  ...
  constexpr PatternPair(Pattern const &pattern, Func const &func) : mPattern{ pattern }, mHandler{ func }
  {
  }
  template <typename Value, typename ContextT> constexpr bool matchValue(Value &&value, ContextT &context) const
  {
...
template <typename Pred> constexpr auto when(Pred &&pred)
{
...
public:
  ...
  template <typename Func> constexpr auto operator=(Func &&func)
  {
...
template <typename... Patterns> constexpr auto ds(Patterns const &...patterns) -> Ds<Patterns...>;
...
public:
  ...
  template <typename Pattern> constexpr auto operator|(OooBinder<Pattern> const &p) const
  {
...
template <typename... Patterns> constexpr auto or_(Patterns const &...patterns)
{
...
public:
  ...
  constexpr static auto matchPatternImpl(Value &&value, Or<Patterns...> const &orPat, int32_t depth, ContextT &context)
  {
    ...
    return std::apply([&value, depth, &context](
                        auto const &...patterns) { return (matchPattern(value, patterns, depth + 1, context) || ...); },
  ...
  {
    return std::apply(
      [depth, idProcess](Patterns const &...patterns) { return (processId(patterns, depth, idProcess), ...); },
      orPat.patterns());
  }
};
...
{
public:
  constexpr App(Unary &&unary, Pattern const &pattern) : mUnary{ std::forward<Unary>(unary) }, mPattern{ pattern }
  {
  }
...
template <typename Unary, typename Pattern> constexpr auto app(Unary &&unary, Pattern const &pattern)
{
...
template <typename Unary, typename Pattern> class PatternTraits<App<Unary, Pattern>>
{
public:
  template <typename Value> using AppResult = std::invoke_result_t<Unary, Value>;
  // We store value for scalar types in Id and they can not be moved. So to
  // support constexpr.
  template <typename Value>
  using AppResultCurTuple = std::conditional_t<std::is_lvalue_reference_v<AppResult<Value>>
                                         ...
                                         ContextT &context)
  {
    ...
    {
      return matchPattern(std::forward<AppResult<Value>>(invoke_(appPat.unary(), value)),
                          appPat.pattern(),
                          depth + 1,
                          context);
    }
...
template <typename... Patterns> constexpr auto and_(Patterns const &...patterns)
{
...
public:
  ...
  {
    constexpr auto patSize = sizeof...(Patterns);
    auto const exceptLast =
      std::apply([&value, depth, &context](
                   auto const &...patterns) { return (matchPattern(value, patterns, depth + 1, context) && ...); },
...
{
public:
  template <typename Value> using AppResultTuple = typename PatternTraits<Pattern>::template AppResultTuple<Value>;
...
template <typename Ptr, typename Value, typename = std::void_t<>> struct StorePointer : std::false_type
{
};
...
template <typename Type> class IdTraits
{
public:
...
template <typename Type> class IdBlockBase
{
  int32_t mDepth;
...
template <typename Type> class IdBlock : public IdBlockBase<Type>
{
public:
  ...
  constexpr decltype(auto) get() const
  {
    return std::visit(
      overload([](Type const &v) -> Type const & { return v; },
               ...
               [](std::monostate const &) -> Type const & { throw std::logic_error("invalid state!"); }),
      IdBlockBase<Type>::mVariant);
  }
};

template <typename Type> class IdBlock<Type const &> : public IdBlock<Type>
{
};

template <typename Type> class IdBlock<Type &> : public IdBlockBase<Type &>
{
public:
  constexpr auto hasValue() const
  {
    return std::visit(overload([](Type *) { return true; }, [](std::monostate const &) { return false; }),
                      IdBlockBase<Type &>::mVariant);
  }
...
public:
  ...
  constexpr decltype(auto) get()
  {
    return std::visit(overload([](Type &v) -> Type & { return v; },
                               [](Type *v) -> Type & {
                                 if (v == nullptr)
                                 {
                                   throw std::logic_error("Trying to dereference a nullptr!");
...
template <typename Type> class IdUtil
{
public:
  template <typename Value>
...
public:
  ...
  {
    return std::visit(overload([](BlockT &v) -> BlockT & { return v; }, [](BlockT *p) -> BlockT & { return *p; }),
                      // constexpr does not allow mutable, we use const_cast
                      // instead. Never declare Id as const.
                      const_cast<BlockVT &>(mBlock));
  }

  template <typename Value> constexpr auto matchValue(Value &&v) const
  {
    if (hasValue())
    {
...
template <typename T> class IsOooBinder<OooBinder<T>> : public std::true_type
{
};

template <typename T> constexpr auto isOooBinderV = IsOooBinder<std::decay_t<T>>::value;

template <typename T> constexpr auto isOooOrBinderV = IsOoo<std::decay_t<T>>::value || isOooBinderV<T>;

template <typename... Patterns> constexpr auto nbOooOrBinderV = ((isOooOrBinderV<Patterns> ? 1 : 0) + ... + 0);

static_assert(nbOooOrBinderV<int32_t &, Ooo const &, char const *, Wildcard, Ooo const> == 2);

template <typename Tuple, std::size_t... I> constexpr size_t findOooIdxImpl(std::index_sequence<I...>)
{
...
using std::get;
template <std::size_t valueStartIdx,
                                                  ...
                                                  std::index_sequence<I...>)
{
  ...
  return (
    func(get<I + valueStartIdx>(std::forward<ValueTuple>(valueTuple)), std::get<I + patternStartIdx>(patternTuple))
    && ...);
}
...
constexpr decltype(auto) matchPatternMultiple(ValueTuple &&valueTuple,
                                              ...
                                              ContextT &context)
{
  return matchPatternMultipleImpl<valueStartIdx, patternStartIdx>(std::forward<ValueTuple>(valueTuple),
                                                                  patternTuple,
                                                                  depth,
                                                                  context,
...
template <std::size_t patternStartIdx,
          std::size_t size,
...
constexpr decltype(auto) matchPatternRange(ValueRangeBegin &&valueRangeBegin,
                                           ...
                                           ContextT &context)
{
  return matchPatternRangeImpl<patternStartIdx>(valueRangeBegin,
                                                patternTuple,
                                                depth,
                                                context,
                                                std::make_index_sequence<size>{});
...
template <std::size_t start, typename Indices, typename Tuple> class IndexedTypes;
...
template <std::size_t start, std::size_t end, class Tuple> using SubTypesT = typename SubTypes<start, end, Tuple>::type;

static_assert(
  std::is_same_v<std::tuple<std::nullptr_t>, SubTypesT<3, 4, std::tuple<char, bool, int32_t, std::nullptr_t>>>);
...
static_assert(std::is_same_v<std::tuple<int32_t, std::nullptr_t>,
...
template <typename T, size_t s> class IsArray<std::array<T, s>> : public std::true_type
{
};

template <typename ValueTuple> constexpr auto isArrayV = IsArray<std::decay_t<ValueTuple>>::value;

template <typename Value, typename = std::void_t<>> struct IsTupleLike : std::false_type
{
};
...
template <typename ValueTuple> constexpr auto isTupleLikeV = IsTupleLike<std::decay_t<ValueTuple>>::value;

static_assert(isTupleLikeV<std::pair<int32_t, char>>);
static_assert(!isTupleLikeV<bool>);

template <typename Value, typename = std::void_t<>> struct IsRange : std::false_type
{
};

template <typename Value>
struct IsRange<Value,
               std::void_t<decltype(std::begin(std::declval<Value>())), decltype(std::end(std::declval<Value>()))>>
    : std::true_type
{
};

template <typename ValueTuple> constexpr auto isRangeV = IsRange<std::decay_t<ValueTuple>>::value;
...
public:
  ...
  template <typename... Ps, typename... Vs> class PairPV<std::tuple<Ps...>, std::tuple<Vs...>>
  {
  public:
    using type = decltype(std::tuple_cat(std::declval<typename PatternTraits<Ps>::template AppResultTuple<Vs>>()...));
  };

  template <std::size_t nbOoos, typename ValueTuple> class AppResultForTupleHelper;

  template <typename... Values> class AppResultForTupleHelper<0, std::tuple<Values...>>
  {
  public:
    using type =
  ...
  template <typename... Values> class AppResultForTupleHelper<1, std::tuple<Values...>>
  {
    ...
    using OooResultTuple =
      typename std::conditional<isBinder, std::tuple<SubrangeT<std::array<ElemT, clippedDiff>>>, std::tuple<>>::type;
  ...
  template <typename RangeType>
  using AppResultForRangeType =
    decltype(std::tuple_cat(std::declval<RangeTuple<RangeType>>(),
                            std::declval<typename PatternTraits<Patterns>::template AppResultTuple<decltype(*std::begin(
                              std::declval<RangeType>()))>>()...));

  template <typename Value, typename = std::void_t<>> class AppResultHelper;

  template <typename Value> class AppResultHelper<Value, std::enable_if_t<isTupleLikeV<Value>>>
  {
  public:
  ...
  template <typename Value> using AppResultTuple = typename AppResultHelper<Value>::type;
    ...
    else if constexpr (nbOooOrBinder == 1)
    {
      ...
      if constexpr (isArray)
      {
        if constexpr (isBinder)
        {
          ...
          result =
            result && matchPattern(std::get<type>(context.back()), std::get<idxOoo>(dsPat.patterns()), depth, context);
        }
      }
      else
      {
  ...
  {
    return std::apply([depth, idProcess](auto &&...patterns) { return (processId(patterns, depth, idProcess), ...); },
                      dsPat.patterns());
  }
};

static_assert(std::is_same_v<typename PatternTraits<Ds<OooBinder<SubrangeT<const std::array<int32_t, 2>>>>>::
                               AppResultTuple<const std::array<int32_t, 2>>,
                             std::tuple<matchit::impl::Subrange<const int32_t *, const int32_t *>>>);

static_assert(
  std::is_same_v<typename PatternTraits<Ds<OooBinder<Subrange<int32_t *, int32_t *>>,
                                           matchit::impl::Id<int32_t>>>::AppResultTuple<const std::array<int32_t, 3>>,
                 std::tuple<matchit::impl::Subrange<const int32_t *, const int32_t *>>>);

static_assert(
  std::is_same_v<typename PatternTraits<Ds<OooBinder<Subrange<int32_t *, int32_t *>>,
                                           matchit::impl::Id<int32_t>>>::AppResultTuple<std::array<int32_t, 3>>,
                 std::tuple<matchit::impl::Subrange<int32_t *, int32_t *>>>);

template <typename Pattern, typename Pred> class PostCheck
{
public:
  constexpr explicit PostCheck(Pattern const &pattern, Pred const &pred) : mPattern{ pattern }, mPred{ pred }
  {
  }
...
template <typename Pattern, typename Pred> class PatternTraits<PostCheck<Pattern, Pred>>
{
public:
  template <typename Value> using AppResultTuple = typename PatternTraits<Pattern>::template AppResultTuple<Value>;
  ...
  template <typename Value, typename ContextT>
  constexpr static auto matchPatternImpl(Value &&value,
                                         PostCheck<Pattern, Pred> const &postCheck,
                                         int32_t depth,
                                         ContextT &context)
  {
...
//                              std::tuple<>>);
static_assert(std::is_same_v<PatternTraits<App<decltype(x), Wildcard>>::template AppResultTuple<std::array<int32_t, 3>>,
...
template <typename T> constexpr auto cast = [](auto &&input) { return static_cast<T>(input); };
...
template <typename Value, typename Variant, typename = std::void_t<>> struct ViaGetIf : std::false_type
{
};
...
struct ViaGetIf<T, Variant, std::void_t<decltype(get_if<T>(std::declval<Variant const *>()))>> : std::true_type
{
};

template <typename T, typename Variant> constexpr auto viaGetIfV = ViaGetIf<T, Variant>::value;

static_assert(viaGetIfV<int, std::variant<int, bool>>);

template <typename T> class AsPointer
{
  static_assert(!std::is_reference_v<T>);

public:
  template <typename Variant, typename std::enable_if<viaGetIfV<T, std::decay_t<Variant>>>::type * = nullptr>
...
template <typename T> constexpr AsPointer<T> asPointer;

template <typename T> constexpr auto as = [](auto const pat) { return app(asPointer<T>, some(pat)); };

template <typename Value, typename Pattern> constexpr auto matched(Value &&v, Pattern &&p)
{
